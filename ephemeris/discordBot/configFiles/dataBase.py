...
class GuildEmojis(BaseModel):
    ...
    class Meta:
        indexes = ((("guild", "emoji_name"), True),)  # Composite primary key
...
class GuildChannelSettings(BaseModel):
    ...
    class Meta:
        primary_key = False
        indexes = ((("guild", "channel_id"), True),)  # Composite primary key
...
class UserEmojis(BaseModel):
    ...
    class Meta:
        indexes = ((("user", "emoji_name"), True),)  # Composite primary key


# Create tables
db.connect()
db.create_tables(
    [GuildSettings, GuildEmojis, GuildChannelSettings, UserSettings, UserEmojis]
)
...
def update_guild_settings(guild_id, guild_data):
    ...
    guild_settings, created = GuildSettings.get_or_create(
        guild_id=guild_id, defaults=guild_data
    )
    ...
    for emoji_name, emoji_value in guild_data.get("emojis", {}).items():
        GuildEmojis.insert(
            guild=guild_settings, emoji_name=emoji_name, emoji_value=emoji_value
        ).on_conflict(
    ...
    for channel_id, settings in guild_data.get("channels", {}).items():
        ...
        GuildChannelSettings.insert(
            ...
            conflict_target=(
                GuildChannelSettings.guild,
                GuildChannelSettings.channel_id,
            ),
            preserve=(
                GuildChannelSettings.use_emojis,
                GuildChannelSettings.allow_filters,
                GuildChannelSettings.whitelisted_users_only,
                GuildChannelSettings.filters,
            ),
        ).execute()
...
def update_user_settings(user_id, user_data):
    ...
    user_settings, created = UserSettings.get_or_create(
        user_id=user_id, defaults=user_data
    )
    if not created:
    ...
    for emoji_name, emoji_value in user_data.get("emojis", {}).items():
        UserEmojis.insert(
            user=user_settings, emoji_name=emoji_name, emoji_value=emoji_value
        ).on_conflict(
...
def newGuildSettings(
    interaction, use_emojis=0, allow_filters=0, whitelisted_users_only=0
) -> dict:
    try:
        return {
...
if __name__ == "__main__":
    ...
    userSettingsTransfer = {
        "109931759260430336": {"username": "figureskate", "expiration": -1},
        "440192160256491521": {"username": "bigdrew", "expiration": 0},
        "110109604423163904": {"username": "ashnel", "expiration": -1},
        "624193482424320000": {"username": "the_cthaeh", "expiration": -1},
        "245939106029240320": {"username": "canigetameow", "expiration": 1724612473},
        "1013019511554834452": {"username": "hitech1497", "expiration": 1722321788},
        "889298297498501151": {"username": ".luckyseven.", "expiration": 1728070376},
        "563525333257551902": {"username": "vanmorten", "expiration": 1723235990},
        "471797895440629767": {"username": "athenakazuo", "expiration": 1729914000},
        "188911699665879041": {"username": "joedread", "expiration": 1753248016},
        "339373441729953802": {"username": "joker10145", "expiration": 1727915206},
        "477287995158953995": {"username": "funtcase1", "expiration": 1727919551},
    }
    ...
    for user_id, s in userSettingsTransfer.items():
        ...
        userData = newUserSettings(
            user_id=user_id, username=s["username"], expiration=s["expiration"]
        )
